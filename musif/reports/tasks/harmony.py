...
from musif.reports.constants import (COMMON_DF, EXCEPTIONS, IMAGE_EXTENSION,
                                     NARROW, NORMAL_WIDTH, TOTAL_ANALYSED,
                                     VISUALIZATIONS, not_used_cols)
...
from musif.reports.utils import (create_excel, get_excel_name,
                                 ...
                                 save_workbook)
from musif.reports.visualizations import (bar_plot_extended,
                                          bar_plot_percentage,
                                          line_plot_extended)
...
def Print_Harmonic_Data(factor: int,_cfg: Configuration, info: DataFrame, additional_info: DataFrame, groups: Dict, workbook: openpyxl.Workbook) -> None:
    ...
    if visualizations_harmony:
        create_visualizations(factor, data_total, ['Bars', 'Beats'], groups, 'Harmonic Rhythm', type='extended')
        create_visualizations(factor, data_total, [i.replace(CHORD_TYPES_prefix,'') for i in chordTypes],
                              groups, 'Chord Types')
        create_visualizations(factor, data_total, [i.replace(ADDITIONS_prefix,'') for i in additions], groups, 'Additions')

        
def Print_Chords(factor: int, _cfg: Configuration, info: dict, groups: dict, additional_info: DataFrame, workbook: openpyxl.Workbook) -> None:
...
def Print_Keys(factor: int, _cfg: Configuration, info: dict, groups: dict, additional_info: DataFrame, workbook: openpyxl.Workbook) -> None:
    data=info['key_areas']
...
def Print_Functions(factor: int, _cfg: Configuration, info: DataFrame, groups: dict, additional_info: DataFrame, workbook: openpyxl.Workbook)-> None:
    data=info['functions']
    ...
    if visualizations_harmony:
        ...
        data_total=pd.concat((info['common_df'], data), axis=1)
        create_visualizations(factor, data_total, third_columns_names, groups, 'Functions')

def Print_Triple_Excel(name: str, factor: int, _cfg: Configuration, groups: dict, additional_info: DataFrame, workbook: openpyxl.Workbook, data_general: DataFrame, data1: DataFrame, data2: DataFrame, data3: DataFrame, third_columns_names: List[str], third_columns_names2: List[str], third_columns_names3: List[str], second_column_names: List[str], second_column_names2: List[str], second_column_names3: List[str])-> None:
...
def Print_Double_Excel(name: str, factor: int, _cfg: Configuration, groups: dict, additional_info: dict, workbook: openpyxl.Workbook, data_general: DataFrame, data1: DataFrame, data2: DataFrame, third_columns_names: list, third_columns_names2: list, second_column_names: List[str]=None, second_column_names2: List[str]=None)-> None:
    computations = ["sum"] + ['mean'] * (len(third_columns_names))    
    computations2 = ['sum'] + ['mean'] * (len(third_columns_names2))
...
def Process_data(data: DataFrame, prefix1: str, prefix2: str, prefix3: str, sorting_list1: list, sorting_list2: list, sorting_list3: list) -> None:
    data.columns= [i.replace('_Count','') for i in data.columns]
...
def create_visualizations(factor: int, data: DataFrame, columns: list, groups: dict, name: str, type: str = 'percentage') -> None:
        ...
        if groups:
            ...
            for g, datag in data_grouped:
                visualisations_path = name + \
                    VISUALIZATIONS + str(g.replace('/', '_'))
            ...
            if type=='extended':
                ...
                bar_plot_percentage(name_bar, data, columns,
                                    name, title)

...
"""
        ...
        w: white
"""
MARKERS = [i for i in Line2D.filled_markers if str(i) not in ["None", "", " ", ","]]
LINESTYLES = [i for i in Line2D.lineStyles.keys() if str(i) not in ["None", "", " "]]
        ...
        for j, info in enumerate(columns_box):
            ...
            for i, group in data[info]:
                ...
                boxes.append(
                    ax[j].boxplot(
                        info_data,
                        labels=column_names[j],
                        notch=False,
                        patch_artist=True,
                        boxprops=dict(color=COLORS[counter]),
                    )
                )
            ...
            if j == 0:
                labels_names = [
                    pitch.Pitch(int(float(x))).unicodeNameWithOctave for x in labels
                ]
                ax[j].set_ylabel("Note")
            else:
                labels_names = [
                    interval.Interval(float(x)).directedName
                    if float(x) >= 0
                    else "Unknown"
                    for x in labels
                ]
...
def melody_bar_plot(
    name: str, data: DataFrame, column_names: List[str], second_title: str = None
):
    size = len(column_names)
    fig = plt.figure(
        figsize=(
            size if len(column_names) > 6 else 10,
            size if len(column_names) > 6 else 10,
        )
    )
    if hasattr(data, "groups"):
        ...
        plt.bar(
            x=range(0, size, 1),
            height=list(list_sum / len(data[column_names])),
            color="b",
            edgecolor="k",
        )
...
def bar_plot_percentage(
    ...
    if hasattr(data, "groups"):
        ...
        for i, group in data[column_names]:
            ...
            valid_results = group[column_names].iloc[
                :, group[column_names].apply(lambda x: x > 2).values[0]
            ]
            size = valid_results.shape[1]

            plt.plot(
                range(0, size, 1),
                valid_results.values[0],
                color=COLORS[color_counter],
                linestyle=LINESTYLES[np.random.randint(len(LINESTYLES))],
                marker=MARKERS[np.random.randint(len(MARKERS))],
                label=str(i),
            ...
            longest_columns = (
                ...
                if len(valid_results.columns) > len(longest_columns)
                else longest_columns
            )
            color_counter += 1
        ...
        # Only elements with more than 2% presence
        valid_results = data[column_names].iloc[
            :, data[column_names].apply(lambda x: x > 2).values[0]
        ]
        size = valid_results.shape[1]
        ...
        plt.bar(
            x=range(0, size, 1),
            height=valid_results.columns,
            color=CSS_COLORS[np.random.randint(len(CSS_COLORS))],
            edgecolor="k",
        )
        plt.draw()
...
def double_bar_plot(name: str, data: DataFrame, second_title: str = None):
    plots = [
        ["LeapsAll", "StepwiseMotionAll"],
        ["PerfectAll", "MajorAll", "MinorAll", "AugmentedAll", "DiminishedAll"],
    ]
        ...
        for k, group in data:
            for i, plot in enumerate(plots):
                ...
                ax[i].set_xticklabels(
                    [p.replace("All", "") for p in plot], rotation=45, ha="right"
                )
    ...
    if hasattr(data, "groups"):
        for k, group in data:
            ...
            for j, info in enumerate(plots):
                ...
                ax[i][j - subtraction].pie(
                    [sum_augmented, sum_descending],
                    autopct="%1.0f%%",
                    colors=[SOFT_COLORS[counter], SOFT_COLORS[counter + 1]],
                )
            ...
            second_title = str(k)
            plt.suptitle(
                "Presence of ascending and descending intervals within each type\n"
                + ("\n" + second_title if second_title is not None else "")
            )
            plt.tight_layout()
            plt.subplots_adjust(top=0.85)
            sub_name = (
                name.replace(".png", "") + "_" + str(k).replace("/", "__") + ".png"
            )
        ...
        for j, info in enumerate(plots):
            ...
            ax[i][j - subtraction].pie(
                [sum_augmented, sum_descending], autopct="%1.0f%%"
            )
            ax[i][j - subtraction].axis("equal")
...
def customized_plot(name, data, column_names, subtitile, second_title=None):
    ...
    for i, cname in enumerate(column_names):
        if cname in possitions:
            ...
            size_bigger = size * 7
            ax.plot(
                loc[0], loc[1], marker="o", markersize=size_bigger, color=colors[cname]
            )
    ...
    if hasattr(data, "groups"):
        ...
        for i, group in data[column_names]:
            plt.bar(
                x=X,
                width=barWidth,
                height=list(group[instr].mean(axis=0, skipna=True)),
        ...
        plt.xticks(
            ticks=[
                r + len(data[column_names]) * barWidth / 2 - barWidth / 2
                for r in range(size)
            ],
            labels=column_names,
        )
...
def line_plot_extended(
    ...
    if hasattr(data, "groups"):
        ...
        counter = 0
        for i, group in data[column_names]:
            results = np.array(group[column_names].mean(axis=0, skipna=True)).astype(
                np.double
            )
        ...
        plt.plot(
            range(0, size, 1),
            list(data[column_names].mean(axis=0, skipna=True)),
            color=COLORS[np.random.randint(len(COLORS))],
            linestyle="--",
            marker="o",
            markersize=15,

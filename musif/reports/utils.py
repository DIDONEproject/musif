...
def create_excel(
    sheet: ExcelWriter,
    rows_groups: Dict[str, list],
    columns: list,
    data: DataFrame,
    third_columns: list,
    computations_columns: list,
    sorting_lists: list,
    groups: list = None,
    first_columns: list = None,
    second_columns: list = None,
    per: bool = False,
    average: bool = False,
    last_column: bool = False,
    last_column_average: bool = False,
    columns2: list = None,
    data2: DataFrame = None,
    third_columns2: list = None,
    computations_columns2: list = None,
    first_columns2: list = None,
    second_columns2: list = None,
    columns3: list = None,
    data3: DataFrame = None,
    third_columns3: list = None,
    computations_columns3: list = None,
    first_columns3: list = None,
    second_columns3: list = None,
    ...
    if groups == None:
        row_iteration(
            ...
            last_column_average=last_column_average,
            columns2=columns2,
            data2=data2,
            third_columns2=third_columns2,
            computations_columns2=computations_columns2,
            first_columns2=first_columns2,
            second_columns2=second_columns2,
            columns3=columns3,
            data3=data3,
            third_columns3=third_columns3,
            computations_columns3=computations_columns3,
            first_columns3=first_columns3,
            second_columns3=second_columns3,
        ...
        for group, group_data in data_grouped:
            ...
            for i, g in enumerate(group):
                if last_printed[i][0] != g:
                    ...
                    if last_printed[i][0] != "":
                        sheet.merge_cells(
                            start_row=last_printed[i][1],
                            start_column=i + 1,
                            end_row=row_number - 2,
                            end_column=i + 1,
                        )
...
def save_workbook(
    path: str, workbook: ExcelWriter, cells_size: str = NORMAL_WIDTH
) -> None:
...
def get_groups_add_info(data: DataFrame, row: int, additional_info: dict):
    if type(additional_info) == list:
        additional_info = [
            ai for ai in additional_info if ai in list(data.columns) and ai != row
        ]
...
def columns_alike_our_data(
    third_columns_names: List[str],
    second_column_names: List[str],
    first_column_names: List[str] = None,
):
    columns = []
    counter_first = 0
    sub_counter_first = 0
    ...
    for c in third_columns_names:
        if first_column_names:
            cn = (
                first_column_names[counter_first][0]
                + second_column_names[counter_second][0]
                + c
            )
            sub_counter_first += 1
...
def print_columns_titles(
    sheet: ExcelWriter, row: int, column: int, column_names: List[str]
):
...
def print_averages_total(
    ...
    for i, v in enumerate(values):
        if (
            exception and i == exception
        ):  # unicamente ocurre en % Trimmed en Melody_values
            sheet.cell(row, values_column).value = (
                str(round(v * 100, 3)).replace(",", ".") + "%"
            )
        else:
            sheet.cell(row, values_column).value = (
                str(v).replace(",", ".") if not per else str(v).replace(",", ".") + "%"
            )
        values_column += 1


def print_averages_total_column(
    sheet: ExcelWriter,
    row: int,
    column: int,
    values: List,
...
def write_columns_titles_variable_length(
    sheet: ExcelWriter, row: int, column: int, column_names: List[tuple], filler=None
):
    ...
    for ind in data.index:
        ...
        if "&" in str(names[ind]):
            ...
            for i, _ in enumerate(voices):
                ...
                line[ROLE] = voices[i]
                line[ARIA_ID] = (
                    str(line[ARIA_ID].item()) + alfa[i]
                    if str(line[ARIA_ID].item()) != ""
                    else ""
                )
                line["Total analysed"] = 1 / len(voices)
                line.iloc[:, line.columns.get_loc("Total analysed") + 1 :] = line.iloc[
                    :, line.columns.get_loc("Total analysed") + 1 :
                ].div(len(voices), axis=1)
...
def print_groups(
    ...
    last_column: bool = False,
    last_column_average: bool = False,
    additional_info: DataFrame = None,
    ponderate: bool = False,
    not_grouped_df: DataFrame = None,
):
    ...
    print_columns_titles(
        sheet, row_number, column_number + 1 + len_add_info, third_columns
    )
    row_number += 1
    exception = -1
    ...
    for s, subgroup in enumerate(grouped):
        ...
        for i, c in enumerate(columns):
            ...
            if c == interval.ABSOLUTE_INTERVALLIC_TRIM_RATIO:  # EXCEPTION
                sheet.cell(row_number, cnumber).value = str(round(value * 100, 1)) + "%"
                sheet.cell(row_number, cnumber).font = FONT
                cnumber += 1
                exception = i - 1
            elif not per:
                sheet.cell(row_number, cnumber).value = (
                    str(value) + "%"
                    if ponderate
                    and column_computation == "sum"
                    and c != "Total analysed"
    ...
    if (
        per or last_column
    ):  # These are the two conditions in which we need to transpose column_values
            ...
            rows_values.append(
                round(
                    sum(
                        [
                            lc[i]
                            for x, lc in enumerate(columns_list)
                            if "All" not in keys_columns[x]
                        ]
                    ),
                    3,
                )
    ...
    if per:
        cn = (
            column_number + len_add_info + 2
            if total_analysed_column
            else column_number + len_add_info + 1
        )
        for i in range(len(columns_list)):  # Traverse each column's information
            row_number = starting_row
            sum_column = sum(columns_list[i]) if sum(columns_list[i]) != 0 else 1
            for j in range(len(columns_list[i])):
                ...
                columns_values[keys_columns[i]][j] = (
                    value if str(value) != "nan" else 0
                )  # update the value
                sheet.cell(row_number, cn).value = (
                    str(value) + "%" if str(value) != "nan" else 0
                )  # print the value
    ...
    if last_column:
        if last_column_average:
            for j, vf in enumerate(rows_values):
                divisor = (
                    len(columns_list)
                    - len([c for c in keys_columns if "All" in c])
                    - len([col[j] for col in columns_list if col[j] == 0.0])
                )
                if divisor != 0:
        ...
        print_averages_total_column(
            sheet,
            starting_row,
            cnumber,
            rows_values,
            average=last_column_average,
    ...
    if last_column:
        if average:
            final_values.append(round(sum(rows_values) / len(rows_values), 3))
        else:
...
def row_iteration(
    ...
    for row in rows_groups:  # Geography, Dramma, Opera, Aria, Label, Composer...
        if row in all_columns or any(sub in all_columns for sub in rows_groups[row][0]):
            ...
            if (
                group == None and row not in forbiden
            ):  # it was 'or' and not 'and' before, but change considered neccessary

                sorting = write_title(
                    sheet, rows_groups, row_number, column_number, row
                )
                ...
                if len(rows_groups[row][0]) == 0:  # No subgroups
                    ...
                    row_number, last_column_used = print_groups(
                        sheet,
                        data.groupby(groups_add_info, sort=False),
                        row_number,
                        column_number,
                        columns,
                        third_columns,
                    ...
                    if columns2 != None:  # Second subgroup
                        ...
                        row_number, last_column_used = print_groups(
                            ...
                            data.groupby(groups_add_info, sort=False)
                            if data2 is None
                            else data2.groupby(groups_add_info, sort=False),
                            starting_row,
                            last_column_used + 1,
                            columns2,
                            third_columns2,
                            computations_columns2,
                            first_columns2,
                            second_columns2,
                            per=per,
                            average=average,
                    ...
                    if columns3 != None:  # Third subgroup
                        ...
                        if data3 is not None:
                            data3 = sort_dataframe(data3, row, sorting_lists, sorting)
                        _, _ = print_groups(
                            sheet,
                            data.groupby(groups_add_info, sort=False)
                            if data3 is None
                            else data3.groupby(groups_add_info, sort=False),
                            starting_row,
                            last_column_used + 1,
                            columns3,
                            third_columns3,
                            computations_columns3,
                            first_columns3,
                            second_columns3,
                            per=per,
                            average=average,
                    ...
                    for i, subrows in enumerate(rows_groups[row][0]):
                        if (
                            subrows == None or subrows not in forbiden
                        ) and subrows in all_columns:
                            if "Tempo" in subrows:
                                data[subrows] = data[subrows].fillna("")
                            starting_row = row_number
                            sort_method = sorting[i]
                            sheet.cell(row_number, column_number).value = subrows
                            sheet.cell(row_number, column_number).fill = greenFill
                            sheet.cell(row_number, column_number).font = FONT
                            data = sort_dataframe(
...
def capitalize_instruments(instruments):
    return [instrument[0].upper() + instrument[1:] for instrument in instruments]

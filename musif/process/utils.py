...
def merge_duetos_trios(df: DataFrame) -> None:
    ...
    voice_cols = [col for col in df.columns.values if any(
        voice in col for voice in voices_list_prefixes)]
    ...
    for index in tqdm(multiple_voices.index):
        ...
        for col in columns_to_merge:
            similar_cols = []
            formatted_col = col.replace(get_part_prefix(
                first_voice), generic_sound_voice_prefix)
            for j in range(0, len(all_voices)):
                similar_col = col.replace(get_part_prefix(
                    first_voice), get_part_prefix(all_voices[j]))
...
def _remove_repeated_voices(multiple_voices):
    ...
    multiple_voices = multiple_voices[~multiple_voices.index.isin(
        repeated_voices_indexes)]
...
def merge_single_voices(df: DataFrame) -> None:
    generic_sound_voice_prefix = get_sound_prefix('Voice')
    pinfo('\nJoining voice parts...')
    singer_columns = [i for i in df.columns.values if any(
    ...
    for col in singer_columns:
        ...
        formatted_col = col.replace(
            singer_part+'_', generic_sound_voice_prefix)
        if formatted_col in df:
            continue
        columns_to_merge = [i for i in singer_columns if "_".join(
            i.split('_')[1:]) == generic_col]
...
def delete_columns(data: DataFrame, config: dictConfig) -> None:
    ...
    data.drop([i for i in data.columns if i.endswith(
        tuple(config[ENDSWITH]))], axis=1, inplace=True)
    data.drop([i for i in data.columns if i.startswith(
        tuple(config[STARTSWITH]))], axis=1, inplace=True)
    data.drop([col for col in data.columns if any(substring in col for substring in tuple(
        config['columns_contain']))], axis=1, inplace=True)
...
def delete_exceptions(data) -> None:
    ...
    data.drop(data.columns[data.columns.str.contains(
        get_part_prefix('Vn'))], axis=1, inplace=True)
    ...
    if (FAMILY_INSTRUMENTATION and FAMILY_SCORING) in data:
        data.drop([FAMILY_INSTRUMENTATION, FAMILY_SCORING],
                  axis=1, inplace=True)
...
def join_keys(df: DataFrame) -> None:
    key_SD = [i for i in [KEY_PREFIX + 'IV' + KEY_PERCENTAGE, KEY_PREFIX +
                          'II' + KEY_PERCENTAGE, KEY_PREFIX + 'VI' + KEY_PERCENTAGE] if i in df]
    key_sd = [i for i in [KEY_PREFIX + 'iv' + KEY_PERCENTAGE,
                          KEY_PREFIX + 'ii' + KEY_PERCENTAGE] if i in df]
    key_tonic = [i for i in [KEY_PREFIX + 'I' + KEY_PERCENTAGE,
                             KEY_PREFIX + 'i' + KEY_PERCENTAGE] if i in df]
    key_rel = [i for i in [KEY_PREFIX + 'III' + KEY_PERCENTAGE,
                           KEY_PREFIX + 'vi' + KEY_PERCENTAGE] if i in df]

    total_key = key_rel+key_tonic+key_sd+key_SD
    others_key = [
        i for i in df.columns if KEY_PREFIX in i and i not in total_key and KEY_MODULATORY not in i]

    df[KEY_PREFIX + 'SD' + KEY_PERCENTAGE] = df[key_SD].sum(axis=1)
    df[KEY_PREFIX + 'sd' + KEY_PERCENTAGE] = df[key_sd].sum(axis=1)
    df[KEY_PREFIX + 'SubD' + KEY_PERCENTAGE] = df[KEY_PREFIX +
                                                  'sd'+KEY_PERCENTAGE] + df[KEY_PREFIX+'SD'+KEY_PERCENTAGE]
...
def join_keys_modulatory(df: DataFrame):
    key_SD = [i for i in [KEY_PREFIX+KEY_MODULATORY + 'IV', KEY_PREFIX +
                          KEY_MODULATORY+'II', KEY_PREFIX+KEY_MODULATORY+'VI'] if i in df]
    key_sd = [i for i in [KEY_PREFIX+KEY_MODULATORY +
                          'iv', KEY_PREFIX+KEY_MODULATORY+'ii'] if i in df]
    key_tonic = [i for i in [KEY_PREFIX+KEY_MODULATORY +
                             'I', KEY_PREFIX+KEY_MODULATORY+'i'] if i in df]
    key_rel = [i for i in [KEY_PREFIX+KEY_MODULATORY +
                           'III', KEY_PREFIX+KEY_MODULATORY+'vi'] if i in df]

    total_key_mod = key_rel+key_tonic+key_sd+key_SD
    others_key_mod = [i for i in df.columns if KEY_PREFIX +
                      KEY_MODULATORY in i and i not in total_key_mod]

    df[KEY_PREFIX+KEY_MODULATORY+'SD'] = df[key_SD].sum(axis=1)
    df[KEY_PREFIX+KEY_MODULATORY+'sd'] = df[key_sd].sum(axis=1)
    df[KEY_PREFIX+KEY_MODULATORY+'SubD'] = df[KEY_PREFIX +
                                              KEY_MODULATORY+'sd'] + df[KEY_PREFIX+KEY_MODULATORY+'SD']

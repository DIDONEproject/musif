...
def box_plot(name, data, second_title = None):
    try:
        columns_box = [['LowestIndex', 'HighestIndex'], ['AmbitusLargestSemitones']]
        ...
        try:
            for j, info in enumerate(columns_box):
                ...
                if i == 0:
                    labels_names = [pitch.Pitch(int(x)).unicodeNameWithOctave for x in labels]
                    ax[i].set_ylabel('Note')
                else:
                    labels_names = [interval.Interval(float(x)).directedName for x in labels]
...
def ivalues_bar_plot(name, data, column_names, second_title = None):
    ...
    fig = plt.figure(figsize=(size if len(column_names) > 6 else 10, size if len(column_names) > 6 else 10))
    plt.bar(x = range(0, size, 1), height=list(list_sum / len(data[column_names])), color = 'w', edgecolor = 'k')
...
def double_bar_plot(name, data, second_title = None):
    plots = [['LeapsAll', 'StepwiseMotionAll'], ['PerfectAll', 'MajorAll', 'MinorAll', 'AugmentedAll', 'DiminishedAll']]
    ...
    for i, plot in enumerate(plots):
        ...
        ax[i].set_xticklabels([p.replace('All', '') for p in plot], rotation=45, ha = 'right')
...
def pie_plot(name, data, second_title = None):
    ...
    plots = ['StepwiseMotion', 'Leaps', 'Perfect', 'Major', 'Minor', 'Augmented', 'Diminished']
    ...
    for j, info in enumerate(plots):
        ...
        ax[i][j - resta].pie([sum_augmented, sum_descending], autopct='%1.0f%%')
...
def customized_plot(name, data, column_names, subtitile, second_title = None):
    ...
    for i, cname in enumerate(column_names):
        if cname in possitions:
            ...
            size_bigger = size * 7
            ax.plot(loc[0], loc[1], marker = 'o', markersize=size_bigger, color = colors[cname])
            #ax.annotate(cname, loc, ha="center", va ='center')
            ax.text(loc[0], loc[1], cname, fontsize = 20, verticalalignment = 'center', horizontalalignment = 'center')

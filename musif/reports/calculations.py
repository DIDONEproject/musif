...
def note_mean(pitch_ps: list, round_mean: bool = False) -> str:
    ...
    if type(mean_pitch) == int or mean_pitch.is_integer():
        ...
        if str(mean_pitch) != "nan":
            ...
            value = "-".join(
                [
                    pitch_up.nameWithOctave.replace("-", "b"),
...
def interval_mean(interval_semitones: list) -> str:
    interval_semitones = [i for i in interval_semitones if i != 0.0]
...
def compute_value(
    subgroup_data, c, computation, ponderate_data, not_grouped_information, ponderate
):
    value = 0
    i = 0
    ...
    try:
        if computation == "mean":
            if not ponderate:
                value = round(
                    np.nansum(column_data)
                    / len([i for i in column_data if str(i) != "nan"]),
                    3,
                )
            else:
                s = 0
            ...
            measures = (
                subgroup_data[c + "Measures"] * subgroup_data["NumberOfBeats"]
            )  # Its a multiplication because it like a division to tjhe whole result
            ...
            value = round(np.nanmin(column_data), 3)
        elif computation == "max":
            value = round(np.nanmax(column_data), 3)
        elif computation == "minNote":
            pitch_ps = [
                pitch.Pitch(n).ps if str(n) != "nan" else np.nan for n in column_data
            ]
            min_ps = np.nanmin(pitch_ps)
            pitch_ps = [str(i) for i in pitch_ps]
            value = column_data.tolist()[pitch_ps.index(str(min_ps))]
        elif computation == "maxNote":
            pitch_ps = [
                pitch.Pitch(n).ps if str(n) != "nan" else np.nan for n in column_data
            ]
            max_ps = np.nanmax(pitch_ps)
            pitch_ps = [str(i) for i in pitch_ps]
            ...
            interval_semitones = [
                interval.Interval(n).semitones if str(n) != "nan" else np.nan
                for n in column_data
            ]
            min_sem = float(np.nanmin(interval_semitones))
            interval_semitones = [str(float(i)) for i in interval_semitones]
            value = column_data.tolist()[interval_semitones.index(str(min_sem))]
        elif computation == "maxInterval":
            interval_semitones = [
                interval.Interval(n).semitones if str(n) != "nan" else np.nan
                for n in column_data
            ]
            max_sem = float(np.nanmax(interval_semitones))
            interval_semitones = [str(float(i)) for i in interval_semitones]
            value = column_data.tolist()[interval_semitones.index(str(max_sem))]

        elif computation == "meanInterval":
            if not all([str(i) == "nan" for i in column_data]):
                value = interval.Interval(
                    np.nanmean(
                        [
                            interval.Interval(i).semitones
            ...
            lowest_notes_names = [
                str(a).split(",")[0] if a != 0.0 else np.nan for a in column_data
            ]
            max_notes_names = [
                str(a).split(",")[1] if str(a) != "nan" and a != 0.0 else np.nan
                for a in column_data
            ]
            min_notes_pitch = [
                pitch.Pitch(a).ps if str(a) != "nan" else np.nan
                for a in lowest_notes_names
            ]
            max_notes_pitch = [
                pitch.Pitch(a).ps if str(a) != "nan" else np.nan
                for a in max_notes_names
            ]
            minimisimo = np.nanmin(min_notes_pitch)
            maximisimo = np.nanmax(max_notes_pitch)
            ...
            if ponderate and value != 0 and column_data.name != "Total analysed":
                ...
                for i in range(len(column_data.tolist())):
                    ...
                    total_intervals = np.nansum(
                        [
                            float(not_grouped_information.iloc[i, c])
                            for c in range(len(all_columns))
                            if "All" not in all_columns[c]
                        ]
                    )
...
def compute_average(dic_data: Dict, computation: str):
    value = 0
    computation = computation.replace("_density", "").replace(
        "_texture", ""
    )  # In case we have averages for densities or textures we compute normal average
    try:
        if computation in ["mean", "min", "sum", "max", "absolute", "meanSemitones"]:
            ...
            for data in dic_data:
                semitones = [
                    interval.Interval(n).semitones if str(n) != "nan" else 0
                    for n in data.split("-")
                ]

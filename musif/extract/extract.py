...
class FeaturesExtractor:
    ...
    def from_files(
        self, musicxml_score_files: List[str], parts_filter: List[str] = None
    ...
    def _process_corpus(
        ...
        for musicxml_file in musicxml_files:
            score_data, parts_data, score_features, score_parts_features = self._process_score(
    ...
    def _process_score(
        self, musicxml_file: str, parts_filter: List[str] = None
    ...
    def _get_part_data(self, score_data: dict, part: Part) -> dict:
        ...
        part_abbreviation, sound_abbreviation, part_number = extract_abbreviated_part(
            sound, part, score_data["parts"], self._cfg
        )
    ...
    def _extract_score_features(self, score_data: dict, parts_data: List[dict], parts_features: List[dict]) -> dict:
        ...
        score_features.update(
            custom.get_score_features(score_data, parts_data, self._cfg, parts_features, score_features)
        )
        score_features.update(
        ...
        score_features.update(
            time.get_score_features(score_data, parts_data, self._cfg, parts_features, score_features)
        )
        score_features.update(
            scoring.get_score_features(score_data, parts_data, self._cfg, parts_features, score_features)
        )
        score_features.update(
            lyrics.get_score_features(score_data, parts_data, self._cfg, parts_features, score_features)
        )
        score_features.update(
            interval.get_score_features(score_data, parts_data, self._cfg, parts_features, score_features)
        )
        score_features.update(
            ambitus.get_score_features(score_data, parts_data, self._cfg, parts_features, score_features)
        )
        score_features.update(
            density.get_score_features(score_data, parts_data, self._cfg, parts_features, score_features)
        )
        score_features.update(
            texture.get_score_features(score_data, parts_data, self._cfg, parts_features, score_features)
        )
        return score_features

    def _extract_corpus_features(
        self, corpus_dir: str, scores_data: List[dict], parts_data: List[dict], scores_features: List[dict]
    ) -> dict:
        corpus_features = {"Corpus": corpus_dir}
        corpus_features.update(
            custom.get_corpus_features(scores_data, parts_data, self._cfg, scores_features, corpus_features)
        )
        corpus_features.update(
            key.get_corpus_features(scores_data, parts_data, self._cfg, scores_features, corpus_features)
        )
        corpus_features.update(
            time.get_corpus_features(scores_data, parts_data, self._cfg, scores_features, corpus_features)
        )
        corpus_features.update(
            scoring.get_corpus_features(scores_data, parts_data, self._cfg, scores_features, corpus_features)
        )
        corpus_features.update(
            lyrics.get_corpus_features(scores_data, parts_data, self._cfg, scores_features, corpus_features)
        )
        corpus_features.update(
            interval.get_corpus_features(scores_data, parts_data, self._cfg, scores_features, corpus_features)
        )
        corpus_features.update(
            density.get_corpus_features(scores_data, parts_data, self._cfg, scores_features, corpus_features)
        )

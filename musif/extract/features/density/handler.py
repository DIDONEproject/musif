...
from musif.extract.utils import _get_timesignature_periods, calculate_total_number_of_beats
...
def update_score_objects(score_data: dict, parts_data: List[dict], cfg: Configuration, parts_features: List[dict], score_features: dict):
    ...
    for i, part_data in enumerate(parts_data):
        ...
        if feature_name in features:
            features[feature_name] = mean([features[feature_name],parts_features[i][SOUNDING_DENSITY]])
        else:
            features[feature_name] = parts_features[i][SOUNDING_DENSITY]
            
        feature_name = get_part_feature(part_abbreviation, DENSITY)
        if feature_name in features:
            features[feature_name] = mean([features[feature_name],parts_features[i][DENSITY]])
        else:

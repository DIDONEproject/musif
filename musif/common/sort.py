...
def sort_list(list_to_sort: list, reference_list: list) -> list:
    ...
    others = [elem for elem in list_to_sort if elem not in sort_dictionary]
    
    return sorted(found, key=lambda x: sort_dictionary[x]) + others
...
def sort_dict(dict_to_sort: dict, reference_list: list) -> dict:
    ...
    for key in dict_to_sort:
        if key in reference_list:
            indexes.append(reference_list.index(key))
        else:
            others.append({key: dict_to_sort[key]})
            pwarn('We do not have the appropiate sorting for {}'.format(key))
    indexes = sorted(indexes)
    list_sorted = [{reference_list[i]: dict_to_sort[reference_list[i]]} for i in indexes]
    list_sorted = list_sorted + others
...
def sort_dataframe(data: DataFrame, column: str, sorting_lists: Dict[list, str], key_to_sort: str) -> DataFrame:
    ...
    if key_to_sort == "Alphabetic":
        ...
        form_list = sorting_lists[key_to_sort] 
        indexes = []

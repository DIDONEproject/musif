...
from musif.reports.visualizations import bar_plot_percentage, double_bar_plot, pie_plot
...
def Intervals(
    ...
    try:
        ...
        third_columns_names_origin, third_columns = fix_column_names(
            sorting_list, all_columns, general_cols
        )

        computations = ["sum"] * len(third_columns)
        create_excel(
            workbook.create_sheet("Weighted"),
            rows_groups,
            third_columns,
            data,
            third_columns,
        ...
        if factor >= 1:
            create_excel(
                workbook.create_sheet("Horizontal Per"),
                rows_groups,
                third_columns,
                data,
                third_columns,
        ...
        save_workbook(
            os.path.join(results_path, get_excel_name(pre_string, name)),
            workbook,
            cells_size=NARROW,
        )
            ...
            if groups:
                ...
                for g, datag in data_grouped:
                    ...
                    bar_plot_percentage(
                        name_bar,
                        datag,
                        third_columns_names_origin,
                        "Intervals" if "Clef" not in name else "Clefs",
                        title,
                        second_title=str(g),
                    )
            elif factor == 1:
                        ...
                        if len(rows_groups[row][0]) == 0:  # no subgroups
                            ...
                            if data_grouped:
                                bar_plot_percentage(
                                    ...
                                    + str(row).replace("Aria", "").upper()
                                    if "Clef" not in name
                                    else "Clefs" + str(row).replace("Aria", "").upper(),
                                    title,
                                )
                        else:  # with subgroups
                            for i, subrow in enumerate(rows_groups[row][0]):
                                if subrow not in EXCEPTIONS:
                                    ...
                                    bar_plot_percentage(
                                        name_bar + "_" + subrow + IMAGE_EXTENSION,
                                        data_grouped,
                                        third_columns_names_origin,
                                        "Intervals"
                                        + str(row).replace("Aria", "").upper()
                ...
                name_bar = path.join(
                    visualizations_path, name.replace(".xlsx", IMAGE_EXTENSION)
                )
...
def fix_column_names(sorting_list, all_columns, general_cols):
    ...
    third_columns_names_origin = sort_list(third_columns_names_origin, sorting_list)
...
def Intervals_types(
    ...
    try:
        data.columns = [
            c.replace("Desc", "Descending").replace("Asc", "Ascending")
        ...
        if visualizations:
            ...
            if groups:
                ...
                for g, datag in data_grouped:
                    ...
                    name_cakes = path.join(
                        result_visualisations, name.replace(".xlsx", "") + "_AD.png"
                    )
                ...
                for row in rows_groups:
                    name_folder = path.join(
                        visualizations_path, "Per_" + row.replace("Aria", "").upper()
                    )
                    ...
                    name_cakes = (
                        name.replace(".xlsx", "").replace("1_factor", "")
                        + "_Per_"
                        + str(row.replace("Aria", "").upper())
                        + "_AD"
                        + IMAGE_EXTENSION
                        ...
                        if len(rows_groups[row][0]) == 0:  # no sub-groups
                            ...
                            if data_grouped:
                                ...
                                double_bar_plot(
                                    name_bars,
                                    data_grouped,
                                    "Per " + str(row.replace("Aria", "").upper()),
                                )
                ...
                name_cakes = path.join(
                    visualizations_path, name.replace(".xlsx", "") + "_AD.png"
                )

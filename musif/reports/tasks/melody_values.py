...
def melody_values(
    ...
    try:
        ...
        PrintStatisticalValues(
            _cfg, data, additional_info, groups, workbook, rows_groups
        )
        PrintAmbitus(
            _cfg, data, additional_info, remove_columns, groups, workbook, rows_groups
        )
        ...
        if visualizations:
            ...
            if groups:
                ...
                for g, datag in data_grouped:
                    path_visualizations = os.path.join(path_visualizations, g)
                    if not os.path.exists(path_visualizations):
                    ...
                    name_box = path.join(
                        path_visualizations,
                        "Ambitus" + name.replace(".xlsx", IMAGE_EXTENSION),
                    )
                ...
                for row in rows_groups:
                    plot_name = (
                        name.replace(".xlsx", "")
                        + "_Per_"
                        + str(row.replace("Aria", "").upper())
                        + IMAGE_EXTENSION
                    )
                    name_bar = path.join(
                        path_visualizations, "Per_" + row.replace("Aria", "").upper()
                    )
                        ...
                        if len(rows_groups[row][0]) == 0:  # no sub-groups
                            ...
                            if data_grouped:
                                ...
                                if row == CLEF1:  # Exception for boxplots
                                    name_box = name_bar.replace(
                                        "Melody_Values", "Ambitus"
                                    )
                                    box_plot(
                                        name_box,
                                        data_grouped,
                            ...
                            for _, subrow in enumerate(rows_groups[row][0]):
                                if subrow not in EXCEPTIONS:
                                    name_bar = (
                                        name_bar.replace(IMAGE_EXTENSION, "")
                                        + "_"
                                        + subrow
                                        + IMAGE_EXTENSION
                                    )
...
def PrintLargestLeaps(_cfg, data, additional_info, groups, workbook, rows_groups):
    ...
    create_excel(
        workbook.create_sheet("Largest_leaps"),
        rows_groups,
        columns,
        data,
        third_columns_names,
...
def PrintAmbitus(
    ...
    first_column_names = (
        ...
        else [("", 1), ("Lowest", 2), ("Highest", 2), ("Ambitus", 2)]
    )
    ...
    computations = (
        ...
        if not remove_columns
        else ["sum", "minNote", "min", "maxNote", "max", "max", "maxInterval"]
    )
    ...
    create_excel(
        ...
        _cfg.sorting_lists,
        groups=groups,
        first_columns=first_column_names,
        second_columns=second_column_names,
        average=True,
        additional_info=additional_info,
    )
...
def PrintStatisticalValues(_cfg, data, additional_info, groups, workbook, rows_groups):
    ...
    data.rename(
        columns={
            "IntervallicMean": INTERVALLIC_MEAN,
            "AbsoluteIntervallicMean": ABSOLUTE_INTERVALLIC_MEAN,
        },
        inplace=True,
    )
    ...
    create_excel(
        workbook.create_sheet("Statistical_values"),
        rows_groups,
        column_names,
        data,
        column_names,
...
def Rename_columns(data):
    ...
    data["MeanSemitones"] = [
        interval.Interval(i).semitones if str(i) != "nan" else np.nan
        for i in data["MeanInterval"]
    ]
    data.rename(
        columns={
            ambitus.LOWEST_NOTE_INDEX: "LowestIndex",
            HIGHEST_NOTE_INDEX: "HighestIndex",
        },
        inplace=True,
    )
    data.rename(
        columns={
            INTERVALLIC_MEAN: INTERVALLIC_MEAN,
            TRIMMED_ABSOLUTE_INTERVALLIC_MEAN: TRIMMED_INTERVALLIC_MEAN,
            ABSOLUTE_INTERVALLIC_TRIM_DIFF: DIFF_TRIMMED,
            ABSOLUTE_INTERVALLIC_MEAN: ABSOLUTE_INTERVALLIC_MEAN,
            INTERVALLIC_STD: STD,
            ABSOLUTE_INTERVALLIC_STD: ABSOLUTE_STD,
            ABSOLUTE_INTERVALLIC_TRIM_RATIO: TRIM_RATIO,
        },
        inplace=True,
    )
    ...
    data.rename(
        columns={
            LARGEST_INTERVAL_ASC: "AscendingInterval",
            ASCENDING_INTERVALLIC_MEAN: "AscendingSemitones",
            LARGEST_INTERVAL_DESC: "DescendingInterval",
            DESCENDING_INTERVALLIC_MEAN: "DescendingSemitones",
        },
        inplace=True,
    )

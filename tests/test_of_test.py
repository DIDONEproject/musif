...
check_functions = {
        ...
        'all_str': lambda x, y: all(is_string_dtype(x[col]) for col in x.columns),
        'all_int': lambda x, y: all(is_integer_dtype(x[col]) for col in x.columns),
        'all_float': lambda x, y: all(is_float_dtype(x[col]) for col in x.columns),
        'any_str': lambda x, y: any(is_string_dtype(x[col]) for col in x.columns),
        'any_int': lambda x, y: any(is_integer_dtype(x[col]) for col in x.columns),
        'any_float': lambda x, y: any(is_float_dtype(x[col]) for col in x.columns),

...
def remove_folder_contents(path: str) -> None:
    """
    Given a folder path, removes all files and directories placed in it. 
    Used primarly to delete visualizations generated by ReportsGenerator class. 
    Parameters
    ----------
    path: str
        Path to folder
...
def _measure_ranges(instrument_measures: int, init: int, end: int, iteration: int = None, offset: int = None, twocompasses_flag: bool = False, remove_repetition_marks_flag: bool = False) -> list:
    measures = []
    o = offset
...
def _find_init_and_end_indexes(instrument_measures: list, init: int, end: int) -> Union[int, int]:
...
def _get_beat_position(beats_timesignature: int, number_of_beats: int, pos: int) -> float:
...
def _expand_repeat_bars(score):
    ...
    for instr in score.parts:
        part_measures = _get_instrument_elements(instr.elements) #returns the measures with repetitions
        ...
        _append_repetitions(final_score, exist_repetition_bars, instr, part_measures, last_measure, part_measures_expanded, repetition_bars, start)
        
    return final_score if exist_repetition_bars else score

def _append_repetitions(final_score, exist_repetition_bars, instr, part_measures, last_measure, part_measures_expanded, repetition_bars, start):
    if exist_repetition_bars:
        p = m21.stream.Part()
        p.id = instr.id
        p.partName = instr.partName
        for repetition_bar in repetition_bars:
            measure = _measure_ranges(part_measures, repetition_bar[0], repetition_bar[0])[0].quarterLength
            if repetition_bar[1] == "start":
                start = _add_start(part_measures, part_measures_expanded, start, repetition_bar, measure)
            elif repetition_bar[1] == "end":
                start = _add_end(part_measures, part_measures_expanded, repetition_bars, start, repetition_bar, measure)
        if start < last_measure:
            ...
            part_measures_expanded.append(_measure_ranges(part_measures, start, last_measure + 1, offset = offset+ measure))

        p.elements = list(itertools.chain(*tuple(part_measures_expanded)))
        final_score.insert(0, p)

def _add_end(part_measures, part_measures_expanded, repetition_bars, start, repetition_bar, measure):
    if len(part_measures_expanded) > 0:
        offset = part_measures_expanded[-1][-1].offset
    else:
        offset = 0
    casilla_1 = True if any(re[1] == '1' and re[0] <= repetition_bar[0] for re in repetition_bars) else False
    casilla_2 = None
    if casilla_1:
        casilla_2 = [re for re in repetition_bars if re[1] == '2' and re[0] > repetition_bar[0]]
        casilla_2 = None if len(casilla_2) == 0 else casilla_2[0] 
    part_measures_expanded.append(_measure_ranges(part_measures, init = start, end = repetition_bar[0], offset = offset+ measure, remove_repetition_marks_flag=True)) # This should erase the repetition marks
    if casilla_2 != None:
        part_measures_expanded.append(_measure_ranges(part_measures, start, casilla_2[0], iteration = 2, offset = part_measures_expanded[-1][-1].offset+ measure))
        start = casilla_2[0] + 1
    else:
        part_measures_expanded.append(_measure_ranges(part_measures, init = start, end = repetition_bar[0], offset = part_measures_expanded[-1][-1].offset + measure))
...
def _add_start(part_measures, part_measures_expanded, start, repetition_bar, measure):
    if len(part_measures_expanded) > 0:
        ...
        offset = 0
    start_measures = _measure_ranges(part_measures, start, repetition_bar[0] - 1, offset = offset + measure)
...
def _get_measures_list(part_measures: list, repeat_elements: list):
    ...
    if there_is_segno:
        ...
        measures_list.append(_measure_ranges(part_measures,1 if not startsin0 else 0, f - 1, iteration = 1 if repeat_bracket else None))
    else:
        measures_list.append(_measure_ranges(part_measures,1 if not startsin0 else 0, len(part_measures), iteration = 1 if repeat_bracket else None))
    ...
    for repeat in repeat_elements:
        ...
        if repeat[1] == "segno":
            ...
            segno_part = _measure_ranges(part_measures, s, f - 1 if there_is_fine else len(part_measures), iteration = 1 if repeat_bracket else None, offset = offset + compass, remove_repetition_marks_flag = True)
            ...
            """if len(repeat_measure) > 0:
                twoCompasses = True"""
            offset = measures_list[-1][-1].offset
            fine_part = _measure_ranges(part_measures, f, len(part_measures), offset = offset + compass, twocompasses_flag=twoCompasses, remove_repetition_marks_flag = True)
            ...
            alsegno_list = _measure_ranges(part_measures, s, f - 1 if there_is_fine else len(part_measures), iteration = 2 if repeat_bracket else None, offset=offset + compass, remove_repetition_marks_flag = True)
            ...
            if startsin0 and there_is_fine and not repeat_bracket:
                f += 1
            dacapo_list = _measure_ranges(part_measures, 0 if startsin0 else 1, f - 1 if there_is_fine else len(part_measures), iteration = 2 if repeat_bracket else None, offset=offset + compass, remove_repetition_marks_flag = True)
            measures_list.append(dacapo_list)

    return measures_list

def _expand_part(part, repeat_elements: list):
    part_measures = _get_instrument_elements(part.elements) #returns the measures with repetitions
    p = m21.stream.Part()
    p.id = part.id
    p.partName = part.partName
    
    part_measures_expanded = _get_measures_list(part_measures, repeat_elements) #returns the measures expanded

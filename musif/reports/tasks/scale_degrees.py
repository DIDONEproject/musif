...
def Emphasised_scale_degrees(
    ...
    try:
        ...
        third_columns_names_origin = sort_list(
            third_columns_names_origin, _cfg.sorting_lists["ScaleDegrees"]
        )
        third_columns_names = ["Total analysed"] + third_columns_names_origin
        third_columns_names2 = ["Total analysed"] + ["1", "3", "4", "5", "7", "Others"]

        computations = ["sum"] * len(third_columns_names)
        computations2 = ["sum"] * len(third_columns_names2)

        emph_degrees = pd.DataFrame(
            prepare_data_emphasised_scale_degrees_second(
                data, third_columns_names, third_columns_names2
            )
        )
        data2 = pd.concat(
            [data[[gc for gc in general_cols if gc in all_columns]], emph_degrees],
            axis=1,
        )
        ...
        if visualizations:
            subtitle = (
                "in relation to the global key"
                if not relative
                else "in relation to the local key"
            )
            ...
            if groups:
                ...
                for g, datag in data_grouped:
                    ...
                    name1 = path.join(
                        ...
                        "Scale_degrees_GlobalKey.png"
                        if not relative
                        else "Scale_degrees_LocalKey.png",
                    )
                    customized_plot(
                        name1,
                        data,
                ...
                customized_plot(name1, data, third_columns_names_origin, subtitle)

    except Exception as e:
        lwarn("{}  Problem found: {}".format(name, e))


def prepare_data_emphasised_scale_degrees_second(
    data: DataFrame, third_columns_names: List[str], third_columns_names2: List[str]
) -> None:
    data2 = {}
    ...
    for name in third_columns_names2:
        ...
        if name == "7" and name in data:  # sumamos las columnas 7 y #7
            ...
            if "#7" in data.columns:
                hastagseven = data["#7"]
                column_data = [
                    np.nansum([seven.tolist()[i], hastagseven.tolist()[i]])
        ...
        if name == "3" and name in data:  # sumamos las columnas 3 y 3b
            ...
            if "b3" in data.columns:
                flat_3 = data["b3"]
                column_data = [
                    np.nansum([three.tolist()[i], flat_3.tolist()[i]])
            ...
            name == "Others"
        ):  # sumamos todas las columnas de data menos 1, 4, 5, 7, #7

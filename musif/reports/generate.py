...
class ReportsGenerator:
    ...
    def generate_reports(
        ...
        self.parts_list = (
            [] if self._cfg.parts_filter is None else self._cfg.parts_filter
        )
        self.visualizations = visualizations
        pinfo(
            colorize("Visualizations are enabled", LEVEL_DEBUG)
            if self.visualizations
            else colorize("Visualizations are disabled", LEVEL_ERROR) + ".\n"
        )
    ...
    def _factor_execution(self, num_factors: int = 0) -> None:
        ...
        self._run_tasks_per_instrument(
            instruments, common_columns_df, num_factors, rg_groups, additional_info
        )
    ...
    def _find_common_columns(self) -> DataFrame:
        ...
        self.metadata_columns = list(
            set(self.metadata_columns) - set(columns_to_remove)
        )
    ...
    def _prepare_part_dataframes(
        self,
        common_columns_df: DataFrame,
        tasks: list,
        ...
        ) = self._find_interval_degree_columns(instrument_level)

        if self._cfg.is_requested_module(interval):
            intervals_info, intervals_types = self._find_interval_columns(
                instrument_level, intervals_list, intervals_types_list
            )
        ...
        if self._cfg.is_requested_module(ambitus):
            try:
                if not self._cfg.is_requested_module(interval):
                    ...
                    tasks["melody_values"] = self._find_melody_columns(
                        instrument_level
        ...
        if instrument == "Voice":
            if self._cfg.is_requested_module(scale):
                tasks["scale"] = self._find_scale_degrees_columns(
                    instrument_level, degrees_list
                ).dropna(how="all", axis=1)
            if self._cfg.is_requested_module(scale_relative):
                tasks["scale_relative"] = self._find_scale_degrees_columns(
                    instrument_level, degrees_relative_list
                ).dropna(how="all", axis=1)
    ...
    def _prepare_common_dataframes(self, instruments: List[str]) -> Tuple[Dict, Dict]:
        common_tasks = {}
        harmony_tasks = {}
        if self._cfg.is_requested_module(density) or self._cfg.is_requested_module(
            texture
        ):
        ...
        if self._cfg.is_requested_module(texture):
            textures_df = self.all_info[
                [i for i in self.all_info.columns if i.endswith("Texture")]
            ].copy()
    ...
    def _run_common_tasks(
        self,
        factor: int,
        common_columns_df: DataFrame,
        common_tasks: Dict[str, str],
        harmony_tasks: Dict[str, str],
        additional_info: Dict[str, str],
        rg_groups: Dict[str, str],
    ) -> None:
        pinfo(
            "\n"
            + "--- Generating common reports: Density, Texture and Harmony ---".center(
                120
            )
        )
        ...
        for groups in rg_groups:
            ...
            if self._cfg.is_requested_module(texture) or self._cfg.is_requested_module(
                density
            ):
                textures_densities_data_path = (
                    path.join(
                        self.main_results_path,
                        "Texture&Density",
                        str(factor) + " factor",
                    )
                    if factor > 0
                    else path.join(self.main_results_path, "Texture&Density", "Data")
                )
                if not os.path.exists(textures_densities_data_path):
                    os.makedirs(textures_densities_data_path)

                self._tasks_execution(
                    self._cfg,
                    groups,
                    additional_info,
                    factor,
                    common_columns_df,
                    results_path=textures_densities_data_path,
                    **common_tasks,
                )
            ...
            if self._cfg.is_requested_module(harmony):
                harmony_data_path = (
                    path.join(
                        self.main_results_path, "Harmony", str(factor) + " factor"
                    )
                    if factor > 0
                    else path.join(self.main_results_path, "Harmony", "Data")
                )
                ...
                self._tasks_execution(
                    ...
                    factor,
                    common_columns_df,
                    results_path=harmony_data_path,
                    **harmony_tasks,
                )

    def _run_tasks_per_instrument(
        self,
        instruments: list,
        common_columns_df: DataFrame,
        num_factors: int,
        ...
        for instrument in tqdm(list(instruments)):
            ...
            self._prepare_part_dataframes(
                common_columns_df, tasks, instrument_level, instrument
            )

            self.results_path_factorx = (
                path.join(
                    self.main_results_path,
                    "Melody_" + instrument,
                    str(num_factors) + " factor",
                ...
                if num_factors > 0
                else path.join(self.main_results_path, "Melody_" + instrument, "Data")
            )
            ...
            for groups in rg_groups:
                try:
                    ...
                    perr(
                        "One or more of the features could not be found in the input dataframe: ".format(
                            e
                        )
                    )
                # else: # from 2 factors
    ...
    def _get_clefs(self, common_columns_df: DataFrame) -> DataFrame:
        ...
        clefs_set = set(
            [
                str(self.all_info["Clef1"][0]),
                str(self.all_info["Clef2"][0]),
                str(self.all_info["Clef3"][0]),
            ]
        )
        if "nan" in clefs_set:
        ...
        for clef in clefs_set:
            ...
            for r, j in enumerate(clefs_info.iterrows()):
                clefs_info[clef].iloc[r] = float(
                    len(
                        [
                            i
                            for i in clefs_info[["Clef1", "Clef2", "Clef3"]].iloc[r]
                            if i == clef
                        ]
                    )
                )
    ...
    def _extract_instruments(self) -> Set[str]:
        instruments = set([])
        all_instruments = set(
            [inst for aria in self.all_info["Scoring"] for inst in aria.split(",")]
        )
    ...
    def _find_harmony_columns(self):
        ...
        key_areas_df = self.all_info[
            [i for i in self.all_info.columns if KEY_PREFIX in i or KEY_GROUPING in i]
        ]
        functions_dfs = self.all_info[
            [
                i
                for i in self.all_info.columns
                if NUMERALS_prefix in i and i.endswith("_Count")
            ]
            + [
                i
                for i in self.all_info.columns
                if CHORDS_GROUPING_prefix in i and i.endswith("_Count")
            ]
        ]
        chords_df = self.all_info[
            [
                i
                for i in self.all_info.columns
                if CHORD_prefix in i
                and CHORD_TYPES_prefix not in i
                and i.endswith("_Count")
            ]
        ]
    ...
    def _find_scale_degrees_columns(self, catch, degrees_list):
        ...
        scale_degrees_info.columns = [
            c.replace(catch, "").replace("Degree", "").replace("_Count", "")
            for c in scale_degrees_info.columns
        ]
    ...
    def _find_interval_degree_columns(self, Instr_level):
        ...
        for col in self.all_info.columns.values:
            if (
                col.startswith(Instr_level + "Intervals")
                or col.startswith(Instr_level + "Leaps")
                or col.startswith(Instr_level + "Stepwise")
            ) and col.endswith("_Count"):
                intervals_types_list.append(col)
            elif (
                col.startswith(Instr_level + "Interval")
                and "Intervals" not in col
                and not col.endswith("Per")
                and "Intervallic" not in col
            ):
        ...
        intervals_types_list.append(
            Instr_level + interval.constants.REPEATED_NOTES_COUNT
        )
    ...
    def _find_melody_columns(self, catch):
        ...
        melody_values.columns = [
            c.replace(catch, "").replace("_Count", "") for c in melody_values.columns
        ]
    ...
    def _find_interval_columns(self, catch, intervals_list, intervals_types_list):
        ...
        intervals_info.columns = [
            c.replace(catch + "Interval", "").replace("_Count", "")
            for c in intervals_info.columns
        ]
        intervals_types = self.all_info[intervals_types_list]
        intervals_types.columns = [
            c.replace(catch, "").replace("Intervals", "").replace("_Count", "")
            for c in intervals_types.columns
        ]
        return intervals_info, intervals_types

    def _get_instrument_prefix(self, instrument: list):
        if instrument.lower().startswith(
            "vn"
        ):  # Violins are the exception in which we don't take Sound level data
            prefix = "Part"
    ...
    def _get_additional_info_and_groups(self, factor: int) -> Dict[str, str]:
        additional_info = {ARIA_LABEL: [TITLE], TITLE: [ARIA_LABEL]}

        if factor == 0:
            self.rows_groups = {ARIA_ID: ([], "Alphabetic")}

            rg_keys = [
                self.rows_groups_original[r][0]
                if self.rows_groups_original[r][0] != []
                else r
                for r in self.rows_groups_original
            ]
        ...
        rg_groups = [[]]
        if factor >= 2:
            rg_groups = list(permutations(list(forbiden_groups.keys()), factor - 1))[4:]
            ...
            if factor > 2:
                ...
                for g in rg_groups:
                    if ARIA_ID in g:
                        ...
                        if any(p in g_rest for p in prohibited):
                            rg_groups.remove(g)
            rg_groups = [r for r in rg_groups if r[0] in list(self.metadata_columns)]

        return additional_info, rg_groups

    def _tasks_execution(
        self,
        _cfg: Configuration,
        groups: list,
        additional_info,
        ...
        **kwargs,
        ...
        if self.visualizations:
            if os.path.exists(path.join(results_path, VISUALIZATIONS)):
                remove_folder_contents(path.join(results_path, VISUALIZATIONS))
            else:
        ...
        try:
            ...
            if "intervals_info" in kwargs and not kwargs["intervals_info"].empty:
                intervals_info = pd.concat(
                    [common_columns_df, kwargs["intervals_info"]], axis=1
                )
                Intervals(
                    self.rows_groups,
                    self.not_used_cols,
                ...
                Intervals(
                    ...
                    pre_string,
                    "Intervals_absolute",
                    _cfg.sorting_lists["Intervals_absolute"],
                    results_path,
                    self.visualizations,
                    additional_info,
                    groups if groups != [] else None,
            ...
            if "intervals_types" in kwargs:
                intervals_types = pd.concat(
                    [common_columns_df, kwargs["intervals_types"]], axis=1
                )
                Intervals_types(
                    self.rows_groups,
                    self.not_used_cols,
            ...
            if "scale" in kwargs and not kwargs["scale"].empty:
                emphasised_scale_degrees_info_A = pd.concat(
                    [common_columns_df, kwargs["scale"]], axis=1
                )
                Emphasised_scale_degrees(
                    self.rows_groups,
                    self.not_used_cols,
                    factor,
                    _cfg,
            ...
            if "scale_relative" in kwargs:
                emphasised_scale_degrees_info_B = pd.concat(
                    [common_columns_df, kwargs["scale_relative"]], axis=1
                )
                Emphasised_scale_degrees(
                    self.rows_groups,
                    self.not_used_cols,
